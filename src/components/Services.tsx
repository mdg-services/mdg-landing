    ...
    <section id="services" className="relative bg-paper-warm">
      <div className="wrap-full py-24 md:py-32">
        <div className="grid items-end gap-8 md:grid-cols-12">
          <div className="md:col-span-8">
            ...
            <Reveal delay={0.06}>
              <h2 className="mt-6 text-balance text-display text-ink" style={{ fontSize: "clamp(33px, 5vw, 64px)" }}>
                ...
                <span className="text-navy-700">{t.services.headingAccent}</span>
              </h2>
            </Reveal>
          </div>
          <Reveal delay={0.12} className="md:col-span-4">
            ...
            <p className="mt-4 font-mono text-[11px] uppercase tracking-[0.2em] text-ink-muted">
              <span className="deva text-[15px] normal-case tracking-normal text-navy-700">{t.services.labelDeva}</span>
              {/* The Latin gloss only earns its place beside the Devanagari
                  word on the English page; in Hindi it repeats it. */}
              {t.services.labelLatin ? <span className="ml-2">{t.services.labelLatin}</span> : null}
            </p>
          </Reveal>
        </div>
        ...
        <Stagger gap={0.06} className="mt-14 grid gap-4 sm:grid-cols-2 md:mt-16 lg:grid-cols-3">
          {SERVICES.map((s) => {
            // Zipped by id, never by position: the two lists are free to
            // fall out of order.
            const card = t.services.items[s.id];
            ...
            return (
              ...
              >
                ...
                <p className="mt-2.5 text-[14.5px] leading-[1.55] text-ink-soft">{card.blurb}</p>
                ...
                <div className="mt-5 flex items-center gap-2 border-t border-ink-hairline pt-4">
                  ...
                  <span className="font-mono text-[11px] uppercase tracking-[0.12em] text-navy-700">{card.metric}</span>
                </div>
              </motion.article>
            );
          })}
